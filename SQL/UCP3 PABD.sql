...
INSERT INTO Mahasiswa (id_mahasiswa, nim, nama, jurusan) VALUES
    ('1', '20240001', 'Ahmad Fauzi',      'Teknologi Informasi'),
    ('2', '20240002', 'Bela Pertiwi',     'Teknologi Informasi'),
    ('3', '20240003', 'Candra Wijaya',    'Teknologi Informasi'),
    ('4', '20240004', 'Diah Lestari',     'Teknologi Informasi'),
    ('5', '20240005', 'Eko Prasetyo',     'Teknologi Informasi'),
	('6', '20240006', 'Muhammad Fajri',     'Teknologi Informasi');	
...
INSERT INTO Presensi (tanggal, status, id_mahasiswa, id_matakuliah, id_dosen) VALUES
('2026-06-15', 'Hadir', 31, 1, 1),
('2026-06-15', 'Hadir', 32, 1, 1),
('2026-06-15', 'Sakit', 33, 1, 1),
('2026-06-15', 'Izin',  34, 1, 1),
('2026-06-15', 'Alpa',  35, 1, 1),
('2026-06-15', 'Hadir', 36, 1, 1),
('2026-06-18', 'Hadir', 31, 2, 2),
('2026-06-18', 'Hadir', 32, 2, 2),
('2026-06-18', 'Hadir', 33, 2, 2),
('2026-06-18', 'Sakit', 34, 2, 2),
('2026-06-18', 'Hadir', 35, 2, 2),
('2026-06-18', 'Hadir', 36, 2, 2),
('2026-06-22', 'Hadir', 31, 3, 13),
('2026-06-22', 'Izin',  32, 3, 13),
('2026-06-22', 'Hadir', 33, 3, 13),
('2026-06-22', 'Hadir', 34, 3, 13),
('2026-06-22', 'Hadir', 35, 3, 13),
('2026-06-22', 'Alpa',  36, 3, 13);

INSERT INTO Presensi (tanggal, status, id_mahasiswa, id_matakuliah, id_dosen) VALUES ('2026-06-20', 'Izin',  31, 2, 2);

ALTER PROCEDURE sp_GetMahasiswa
AS
BEGIN
    SELECT id_mahasiswa, nim, nama, jurusan FROM Mahasiswa; -- Tambahkan id_mahasiswa
END

EXEC sp_GetMatakuliah;
...
BEGIN
    ...
    VALUES (
        @tanggal, 
        @status, 
        (SELECT id_mahasiswa FROM Mahasiswa WHERE nama = @pnama), 
        @id_mk, 
        @id_dosen
    );
END
...
SELECT id_mahasiswa, nama FROM Mahasiswa;

ALTER PROCEDURE sp_InsertPresensi
    @tanggal DATE,
    @status VARCHAR(10),
    @id_mahasiswa INT, -- Ubah @pnama menjadi @id_mahasiswa
    @id_mk INT,
    @id_dosen INT
AS
BEGIN
    ...
    INSERT INTO Presensi (tanggal, status, id_mahasiswa, id_matakuliah, id_dosen)
    VALUES (@tanggal, @status, @id_mahasiswa, @id_mk, @id_dosen);
END
...
CREATE PROCEDURE sp_GetAllMatakuliah
AS
BEGIN
    SELECT kode_mk, nama_mk, sks FROM Matakuliah
END

select * from Mahasiswa

ALTER PROCEDURE sp_ResetData
AS
BEGIN
    SET NOCOUNT ON;
    
    -- Cek dulu, kalau backup tidak ada, batalkan proses agar data tidak hilang sia-sia
    IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'Mahasiswa_Backup')
    BEGIN
        PRINT 'Backup tidak ditemukan, proses dibatalkan.';
        RETURN;
    END

    -- Hapus data hanya jika backup sudah dipastikan ada
    DELETE FROM Presensi;
    DELETE FROM KRS;
    DELETE FROM Mahasiswa;
...
CREATE PROCEDURE sp_SimulasiHackedNama
    @NIM VARCHAR(20)
AS
BEGIN
    -- Prosedur ini mensimulasikan aksi peretasan data
    -- di mana nama mahasiswa diubah secara paksa
    UPDATE Mahasiswa 
    SET Nama = 'HACKED' 
    WHERE NIM = @NIM;
    
    PRINT 'Simulasi serangan berhasil dijalankan pada NIM: ' + @NIM;
END;
GO